...
Every query was executed against the live workspace on **2026-06-26** (`Verified` = rows returned; default 30d,
wider window noted inline). Legend: ✅ verified with data · ⚠️ valid syntax, 0 events found · ⬜ backlog.
The advanced-feature / EDR-block-mode changes occurred on 2026-05-05, so they confirm over a 180d window.
...
| **Defender XDR — Advanced features (all)** | Alert on any `SetAdvancedFeatures` toggle | [`AdvancedFeatureModificationAlert.kql`](AdvancedFeatureModificationAlert.kql) | 15 (180d) | ✅ |
| **Defender XDR — EDR block mode** | Alert when EDR-in-block-mode is toggled | [`EDRBlockMode.kql`](EDRBlockMode.kql) | 5 (180d) | ✅ |
...
> ⚠️ **MDO** is the only query with no confirming data — 0 events even over 180d (no anti-phish / Safe Links /
> Safe Attachments / spam-policy cmdlets were run in this tenant). Syntax & filters are correct; it will return
> rows once an MDO policy is edited. Validate against a tenant with recent MDO changes if you need a row sample.
